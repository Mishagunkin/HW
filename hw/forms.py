...
class BookingForm(forms.Form):
    user = forms.CharField(disabled=True,label='Арендатор')
    hotel = forms.CharField(disabled=True, label='Автомобиль')
    price = forms.CharField(disabled=True,label='Стоимость')
    start_date = forms.DateField(widget=forms.SelectDateWidget(), label='Дата начала аренды')
    end_date = forms.DateField(widget=forms.SelectDateWidget(), label='Дата конца аренды')
