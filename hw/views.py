    ...
    if request.method == 'POST':
        ...
        if is_val:
            ...
            if models.Hotel.objects.filter(name=data['name']).exists():
                form.add_error('name',['Автомобиль с таким названием уже зарегестрирован'])
                is_val = False
        if is_val:
...
def ajax_last_bookings(request):
    ...
    response =""
    if num == 0:
        response += "<thead><tr><th>Арендовано</th><th>Прибытие</th><th>Отбытие</th><th>Стоимость</th></tr></thead>"
